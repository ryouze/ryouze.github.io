+++
date = 2024-07-08T14:23:09+02:00
...
[FTXUI](https://github.com/ArthurSonzogni/FTXUI) is likely the most popular TUI library for C++. However, its [learn-by-example documentation](https://arthursonzogni.github.io/FTXUI/) omits many details that would be helpful for beginners. This guide fills that gap, first extending upon the [original documentation](https://arthursonzogni.github.io/FTXUI/) and then walking you through creating several TUI applications, from the basics to more advanced projects.
...
Now, while in the `build` directory, generate the build files using the `CMakeLists.txt` in the directory above. This will also download FTXUI, so it may take a while.
...
Ok, we have a simple TUI app up and running.
...
int main()
{
    ...
    Element document =
        vbox({
            paragraph("Lorem ipsum dolor sit amet, consectetur adipiscing elit. Mauris mauris augue, ultricies ac congue eu, vulputate eget nibh. Nullam augue elit, commodo et nibh ac, pretium accumsan nulla. Aliquam et aliquet tortor. Suspendisse vitae vehicula mi, sit amet pulvinar metus."),  // <-- ftxui::paragraph
        }) | border;
...
As per the [original documentation](https://arthursonzogni.github.io/FTXUI/), this can be combined with mouse input, which shall be explained later.
